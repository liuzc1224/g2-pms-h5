...
export class SySMenuComponent implements OnInit {
    ...
    makeNew(){
      ...
      if(formData.parentId)
        postObj['parentId'] = formData.parentId
        
      this.service.addNew(postObj)
        .subscribe(
          res => {
            if(res['success'] === true){
    ...
    showMenu($event){
      ...
      el['style'].top = $event.event.clientX - 120 + 'px' ;
      el['style'].left = $event.event.clientX + 70 + 'px' ;
